...
export function StatusHUD() {
  ...
  const inactiveFailure = !hand.enabled && hand.phase !== 'idle';
  const input = hand.enabled
    ? (hand.tracking ? `${hand.hands.length} HAND${hand.hands.length > 1 ? 'S' : ''}` : hand.phase.toUpperCase())
    : inactiveFailure ? hand.phase.toUpperCase().replaceAll('-', ' ') : 'MOUSE';
  const vision = hand.error
    ? `${hand.phase.toUpperCase().replaceAll('-', ' ')} · ERROR`
    : hand.enabled ? `${hand.delegate ?? '—'} · ${hand.inferenceTime.toFixed(0)} MS · ${hand.inferenceFps.toFixed(0)} FPS` : 'STANDBY';
  return (
    <div className="status-hud">
      ...
      <div><small>CORE</small><b>DOCUMENTS V1.1</b></div>
    </div>
  );
}
